...
As the position given by the pose publisher is referenced to the center of the Gazebo simulation world, this gives a common local positioning between UAVs and the rest of the simulation environment.

This feature, allow the developers to work with **Software-In-The-Loop** simulations without **GNSS transformations**, avoiding to deal with their respective precision errors. 
...
If you yet have installed the previously mentioned [dependencies](#dependencies), then you can go to the [Installation](#installation) section.
...
As we use the PX4 visual navigation feature, a simulated **VIO-based** like the **x500_vision** multirrotor model is needed when running the px4 sitl instance initialization script. 

```
export PX4_SIM_MODEL=x500_vision PX4_SYS_AUTOSTART=4005 ${PX4_FOLDER}/build/px4_sitl_default/bin/px4
``` 

Neither **x500**, nor **x500_mono_cam** can be used to this purpose because of the **odometry publisher** lacking.

In order to make the autopilot to work without GPS sensor fusioning you need to set these parameters to the following values:
...
This tool will inject local positioning by publishing in the `/px4_<n>/fmu/in/vehicle_visual_odometry` ros2 topic where **n** is 1 by default. Remeber to export the **PX4_UXRCE_DDS_NS** parameter with a subsecuent value when launching the SITL simulation with one or multiple PX4 instances.
